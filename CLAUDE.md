...
**Strategic context:** See `STRATEGY.md` for competitive positioning vs. Microsoft FUAM and persona analysis. See `TASKS.md` for the active task backlog.

## Tech Stack
- **Framework:** React 19 + TypeScript 6 (strict)
- **Build:** Vite 8
- **Styling:** Tailwind CSS v4 (via `@tailwindcss/vite` — zero-config, no `tailwind.config.ts`)
- **UI Primitives:** @radix-ui/react-slot + class-variance-authority + tailwind-merge
- **State:** Zustand 5
- **Charts:** Recharts 3
...
- **Analytics:** Plausible or Fathom [PLANNED — no cookies, GDPR-compliant; do not use GA4 as this tool handles client tenant data]

## Project Structure
```
src/
  auth/         → MSAL config, AuthProvider, useAuth hook, ProtectedRoute
  api/          → fabricClient (base HTTP), fabricClientInstance (singleton export),
                  graphClient (MS Graph for group expansion),
  ...
  store/        → Zustand stores (workspace, capacity, security, ui,
                  tenantSettings, widelyShared, activity [PLANNED])
  hooks/        → Custom hooks (currently empty — reserved for shared hook logic)
  components/
    ...
    shared/     → DataTable, StatCard, SearchBar, EmptyState, ErrorBoundary,
                  ExportButton, ItemTypeBadge, PrincipalTypeBadge, LoadingState,
                  StateBadge, Toast
    charts/     → ItemsByTypeChart, WorkspacesByCapacityChart
    security/   → AccessConcentrationChart, EffectiveAccessCard, GroupExpansionRow,
                  SecurityFindingsPanel, SecurityPostureCard, SpnGovernancePanel,
                  SpofWorkspacesPanel, WorkspacePivotTable,
                  TenantSettingsRiskPanel, WidelySharedPanel,
  ...
  utils/        → healthScore, export (CSV), constants (single source of truth),
                  ...
                  tenantSettingRisks [PLANNED], ghostWorkspaces [PLANNED]
```
...
- `PrincipalType` (`'User' | 'Group' | 'ServicePrincipal' | 'ServicePrincipalProfile'`) defined in `src/api/types/roleAssignment.ts`
...
| Group | Examples |
|-------|---------|
| Rate Limiting | `ADMIN_RATE_LIMIT` (200), `ADMIN_RATE_WARNING_THRESHOLD` (180), `DEFAULT_RETRY_AFTER_MS` (5000), `MAX_RETRY_COUNT` (3), `BASE_RETRY_DELAY_MS` (1000), `MAX_RETRY_DELAY_MS` (30000) |
...
| Principal Types | `PRINCIPAL_TYPE_COLORS` (User=#495057, Group=#4F46E5, ServicePrincipal=#7C3AED) |
| Health | `HEALTH_SCORE_WEIGHTS`, `GRADE_THRESHOLDS`, `MAX_REASONABLE_ITEM_COUNT`, `CRITICAL_HEALTH_CHECKS` (['capacity','description','workspaceIdentity']) |
| Security | `ADMIN_ROLE_WARNING_THRESHOLD` (5), `GROUP_EXPAND_INITIAL_COUNT` (10), `ADMIN_ASSIGNMENT_PCT_THRESHOLD` (30) |
| Graph | `GRAPH_SCOPES` (`GroupMember.Read.All`) |
| Pricing | `CU_RATE_PER_HOUR` (0.18 USD) |
| Score Ring | `SCORE_RING_ANIMATION_MS` (600), `SCORE_RING_DEFAULT_SIZE` (120), `SCORE_RING_STROKE_WIDTH` (8) |
| Health Grid | `HEALTH_GRID_TILE_SIZE` (36), `HEALTH_GRID_GAP` (6), `HEALTH_GRID_MAX_TILES` (100), `HEALTH_GRID_HOVER_SCALE` (1.15) |
| Session | `SESSION_IDLE_TIMEOUT_MS` (30min), `SESSION_WARNING_MS` (5min) |
| Validation | `GUID_REGEX`, `HTTP_ERROR_MESSAGES` (keyed by status code), `HTTP_ERROR_MESSAGE_SERVER` |
...
- **Motion:** Functional only. 120ms hover, 200ms transitions, 300ms layout shifts. Base transitions use `ease`; enter animations use `ease-out` (250ms); exits use `ease-in` (150ms). No bounce/overshoot.
...
- Mock data: 3 capacities, 35 workspaces, 200+ items (all 19 item types), 8 users, 4 groups, 2 SPNs with role assignments
- UI behavior:
...
# Admin APIs — Fabric API base (Fabric Admin role required, 200 req/hr limit)
...
GET  /v1/admin/tenantsettings                    # Tenant-wide settings (Feature 2 ✅)
GET  /v1/admin/widelySharedArtifacts/linksSharedToWholeOrganization  # Org-wide shared items (Feature 3 ✅ — uses Power BI base + POWERBI_SCOPES)

# Power BI Admin API — different base URL: https://api.powerbi.com/v1.0/myorg
# Scope: https://analysis.windows.net/powerbi/api/.default
# FabricClient.get()/post() accept optional scopes param — pass POWERBI_SCOPES for these endpoints
GET  /admin/widelySharedArtifacts/linksSharedToWholeOrganization  # Org-wide shared items (Feature 3 ✅)
GET  /admin/activityevents?startDateTime=...&endDateTime=...  # Activity log (PLANNED — Feature 1)
```

All Fabric list endpoints return `{ value: T[], continuationToken?: string }`.
Power BI widely shared artifacts API returns `{ artifactAccessEntities: T[], continuationToken?: string }`.
Power BI activity events API returns `{ activityEventEntities: T[], continuationUri?: string }`.

**Note on Power BI Admin API:** `FabricClient.get()` and `.post()` accept an optional `scopes?: string[]` parameter. Pass `POWERBI_SCOPES` (`['https://analysis.windows.net/powerbi/api/.default']`) when calling `api.powerbi.com` endpoints. MSAL handles consent automatically via `acquireTokenPopup` on first use — no new consent UI needed. The scope blocker for Feature 1 is now resolved.
...
- `/about` route is **public — no auth guard** (ProtectedRoute must not wrap it); serves as SEO surface and consultant onboarding reference
...
## Planned Features (see TASKS.md for full breakdown)

All three Phase 1 features are complete. Build order: 2 ✅ → 3 ✅ → 1 ✅. Next: Phase 2 (Consultant Delivery).

### Feature 2: Tenant Settings Risk Panel ✅ COMPLETE
- API: `GET /v1/admin/tenantsettings` (Fabric Admin base, same scope as existing admin calls)
- Files: `src/api/types/tenantSettings.ts`, `src/api/tenantSettings.ts`, `src/store/tenantSettingsStore.ts`, `src/utils/tenantSettingRisks.ts`, `src/components/security/TenantSettingsRiskPanel.tsx`
- Surfaces enabled high-risk settings (PublishToWeb, ExternalSharingEnabled, etc.) with risk level badges

### Feature 3: Widely Shared Objects Panel ✅ COMPLETE
- API: `GET https://api.powerbi.com/v1.0/myorg/admin/widelySharedArtifacts/linksSharedToWholeOrganization` (Power BI Admin base — NOT Fabric base)
- Scope: `POWERBI_SCOPES` (`analysis.windows.net/powerbi/api/.default`) — passed via `FabricClient.get()` scopes override
- Response envelope: `artifactAccessEntities` (not `value`) — custom pagination loop required
- Files: `src/api/types/widelyShared.ts`, `src/api/widelyShared.ts`, `src/store/widelySharedStore.ts`, `src/components/security/WidelySharedPanel.tsx`
- Side effect: `FabricClient` now has optional per-request `scopes` param — unblocks Feature 1

### Feature 1: Ghost Workspace Detection ✅ COMPLETE (2026-04-05)
- API: `GET https://api.powerbi.com/v1.0/myorg/admin/activityevents` (Power BI Admin base)
- Scope: `POWERBI_SCOPES` via `FabricClient.get(url, POWERBI_SCOPES)` — same pattern as Feature 3
- Response envelope: `activityEventEntities`; pagination uses `continuationUri` (full URL, not token)
- Datetime params wrapped in single quotes per Power BI OData spec: `?startDateTime='...'&endDateTime='...'`
- Cache guard uses `lastFetchedAt` (not array length) — zero-ghost tenants don't re-fetch on every mount
- Files: `src/api/types/activityEvents.ts`, `src/api/activityEvents.ts`, `src/store/activityStore.ts`, `src/utils/ghostWorkspaces.ts`, `src/components/security/GhostWorkspacesPanel.tsx`
- Constants: `GHOST_WORKSPACE_THRESHOLD_DAYS` (90), `ACTIVITY_LOG_LOOKBACK_DAYS` (30) in `// -- Ghost workspaces --` section
